...
```bash
npm install -g @vecode-cli/vecode-cli@preview
```
...
```bash
npm install -g @vecode-cli/vecode-cli@latest
```
...
- [**Memory Management**](./docs/tools/memory.md) - Using VE.md context files
...
- [**Architecture Overview**](./docs/architecture.md) - How VeCLI works
...
Configure MCP servers in `~/.ve/settings.json` to extend VeCLI with custom tools:
...
We welcome contributions! VeCLI is fully open source (Apache 2.0), and we encourage the community to:
